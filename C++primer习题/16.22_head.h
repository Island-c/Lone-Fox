...
public:
	...
	//拷贝控制成员： 
	TextQuery() :words(nullptr,DebugDelete()) {} //默认构造函数
	TextQuery(ifstream &in);
	TextQuery(const TextQuery &rhs) : words(new vector<string>(*(rhs.words)),DebugDelete()), word_No(rhs.word_No) { cout << "拷贝构造函数" << endl; } //拷贝构造函数
	TextQuery(TextQuery &&rhs) : words(new vector<string>(std::move(*(rhs.words))),DebugDelete()), word_No(std::move(rhs.word_No))
	{
		rhs.words = nullptr;
	...
	TextQuery& operator=(const TextQuery &rhs) //拷贝赋值运算符
	{
		...
		words = shared_ptr<vector<string>>(new vector<string>(*rhs.words),DebugDelete());
		word_No = rhs.word_No;
		cout << "拷贝赋值运算符" << endl;
		return *this;
	}
	TextQuery& operator=(TextQuery &&rhs) //移动赋值运算符
	{
		if (this != &rhs)
		{
			words = shared_ptr<vector<string>>(new vector<string>(std::move(*rhs.words)),DebugDelete());
